...
#define ABORT( cp, m )   { printf( "codepoint 0x%04lX %s\n", cp, m ); return -__LINE__; }
  ...
  if( sft_render( sft, gid, i ) < 0 ) ABORT( cp, "not rendered" )

  XGlyphInfo info = {
    .x = (short) -hmtx.leftSideBearing, .y = (short) -e.yOffset,
    .width  = (unsigned short) i.width, .height = (unsigned short) i.height,
    .xOff = (short)hmtx.advanceWidth,   .yOff = 0
  };
  Glyph g = cp;
  XRenderAddGlyphs( dpy, glyphset, &g, &info, 1, i.pixels, (int) (i.width * i.height) );
...
#define END(m) { printf( "%s\n", m ); goto end; }
  ...
  for( XEvent e; !XNextEvent( dpy, &e ); )
  {
    if( e.type == Expose )
    {
      ...
      for( char t[256]; fgets( t, sizeof(t), f ); )
      {
        ...
        for( int i = 0; i < n; i++ ) sft_glyph( &sft, dpy, glyphset, u[i] );
